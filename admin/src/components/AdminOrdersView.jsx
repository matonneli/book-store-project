...
const AdminOrdersView = () => {
    ...
    const { pickUpPoints, orderStatuses, itemStatuses, formatOrderStatus, formatItemStatus, isReady } = useReferences();
    ...
    useEffect(() => {
        ...
        const pickupPointId = activeTab === 'all' ? null : parseInt(activeTab);
        fetchOrders({
            ...filters,
            pickupPointId
        });
        ...
        <div>
            ...
            <ul className="nav nav-tabs mb-3">
                ...
                {pickUpPoints.map(point => (
                    <li className="nav-item" key={point.pickupPointId}>
                        <button
                            className={`nav-link ${activeTab === point.pickupPointId.toString() ? 'active' : ''}`}
                            onClick={() => setActiveTab(point.pickupPointId.toString())}
                        >
                            {point.name}
                        </button>
                    </li>
            ...
            <div className="card">
                <div className="card-header d-flex justify-content-between align-items-center">
                    <h5 className="mb-0">
                        📦 Orders {activeTab === 'all' ? '' : `- ${pickUpPoints.find(p => p.pickupPointId.toString() === activeTab)?.name}`}
                    </h5>
