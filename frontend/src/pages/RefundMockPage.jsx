...
const RefundMockPage = () => {
    ...
    const handleRefund = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/payment/mock-refund?orderId=${orderId}`, {
                method: 'POST',
                headers: {
            ...
            const timer = setInterval(() => {
                setCountdown((prev) => {
                    if (prev <= 1) {
                        clearInterval(timer);
                        if (window.opener) {
                            window.opener.postMessage('refund_success', '*');
                        }
                        window.close();
                        return 0;
                    }
        ...
        return (
            <div className="relative z-10">
                ...
                <p className="text-2xl md:text-3xl drop-shadow-lg mb-8 font-bold" style={{ fontFamily: "'Poppins', sans-serif" }}>
                    Refund amount: {totalAmount} zł
                </p>

                {countdown === null ? (
                    <div className="space-y-4">
                        <p className="text-lg drop-shadow-lg mb-4" style={{ fontFamily: "'Poppins', sans-serif" }}>
                            This is a simulated refund system for demonstration purposes
                        </p>
                        <button
